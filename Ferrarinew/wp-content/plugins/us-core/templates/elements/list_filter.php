...
if ( ! empty( $mobile_width ) ) {
	...
	$output .= '<button class="w-filter-list-closer" title="' . esc_attr( us_translate( 'Close' ) ) . '" aria=label="' . esc_attr( us_translate( 'Close' ) ) . '">';
	$output .= '</button>';
}
...
foreach ( $items as $i => $filter_item ) {
	...
	$output_items .= '<button class="w-filter-item-title' . $_dropdown_field_class . '">';
...
if ( ! empty( $mobile_width ) AND $output_items !== '' ) {
	...
	$output .= '<button class="w-btn w-filter-button-submit ' . us_get_btn_class() . '">';
